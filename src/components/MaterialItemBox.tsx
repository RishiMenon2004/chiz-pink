...
export default function MaterialItemBox({ material, requiredQuantity, canHaveMultiMat = true, hadAddSub }: MaterialItemBoxProps) {
	...
	const contextValue: MultiMatModalContextType = {
		registerMultiMatAmount: (id, callback) => multiMatAmountCallbacks.current.set(id, callback),
		unregisterMultiMatAmount: (id) => multiMatAmountCallbacks.current.delete(id),
	...
	return (
		...
		>
			...
			onClick={handleBoxClick}>
				<Image src={`/materials${material.iconSrc}.png`} width={128} height={128} alt={`${material.name} icon`}/>
			</div>
